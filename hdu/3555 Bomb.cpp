...
题意：给定一个数n，求[1,n]中含有"49"的数的个数。
From 1 to 500, the numbers that include the sub-sequence "49" are "49","149","249","349","449","490","491","492","493","494","495","496","497","498","499",so the answer is 15.

思路：首先我们考虑一个数中有无"49"的几种情况。(假设*中不包含"49")
    1.***49****
    2.*********
    3.9********
    4.49*******
...
则：dp[i][0] = dp[i-1][0] * a[i] - dp[i-1][1];  表示长度为 i 的不含有49的数字的个数，它等于长度为 i - 1 的不含有49的数字的个数*当前位的数字，因为这个位置可以填0~a[i] - 1，然后再减去长度为 i - 1 的最高位是9的数字的个数，因为如果长度为 i - 1 的最高位是9的话，那么高一位就不能填4了，否则就组成了49。
